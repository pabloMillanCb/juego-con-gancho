...
// 	};
	...
	// Physijs.createMaterial   MODIFICADO
//   class createMaterial {
	var createMaterial = function( material, friction, restitution ) {
...
//     }
	...
	// Constraints         MODIFICADO
  class PointConstraint {
// 	PointConstraint = function( objecta, objectb, position ) {
    ...
    // Physijs.HingeConstraint  MODIFICADO
  class HingeConstraint {
// 	HingeConstraint = function( objecta, objectb, position, axis ) {
    ...
    // Physijs.SliderConstraint   MODIFICADO
  class SliderConstraint {
// 	SliderConstraint = function( objecta, objectb, position, axis ) {
    constructor(objecta, objectb, position, axis) {
      if ( axis === undefined ) {
        axis = position;
        position = objectb;
    ...
    // Physijs.ConeTwistConstraint    MODIFICADO
  class ConeTwistConstraint {
// 	ConeTwistConstraint = function( objecta, objectb, position ) {
    constructor(objecta, objectb, position) {
  ...
  class DOFConstraint {
// 	DOFConstraint = function( objecta, objectb, position ) {
    constructor(objecta, objectb, position) {
    ...
    // Physijs.Scene   MODIFICADO
	Scene.prototype._updateConstraints = function( data ) {
    ...
    // Physijs.Scene   MODIFICADO
	Scene.prototype._updateCollisions = function( data ) {
		/**
		 * #TODO
    ...
    // Physijs.Scene   MODIFICADO
	Scene.prototype.onSimulationResume = function() {
		this.execute( 'onSimulationResume', { } );
	};

    // Physijs.Scene   MODIFICADO
	Scene.prototype.removeConstraint = function( constraint ) {
    ...
    // Physijs.Scene   MODIFICADO
	Scene.prototype.execute = function( cmd, params ) {
    ...
    // Physijs.Scene   MODIFICADO
	Scene.prototype.setFixedTimeStep = function( fixedTimeStep ) {
		if ( fixedTimeStep ) {
	...
	// Physijs.Mesh.mass   MODIFICADO
	Mesh.prototype.__defineGetter__('mass', function() {
		return this._physijs.mass;
	});

	// Physijs.Mesh   MODIFICADO
	Mesh.prototype.__defineSetter__('mass', function( mass ) {
	...
	// Physijs.Mesh.applyCentralImpulse   MODIFICADO
	Mesh.prototype.applyCentralImpulse = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyCentralImpulse', { id: this._physijs.id, x: force.x, y: force.y, z: force.z } );
		}
	};

	// Physijs.Mesh.applyImpulse   MODIFICADO
	Mesh.prototype.applyImpulse = function ( force, offset ) {
		if ( this.world ) {
	...
	// Physijs.Mesh.applyTorque   MODIFICADO
	Mesh.prototype.applyTorque = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyTorque', { id: this._physijs.id, torque_x: force.x, torque_y: force.y, torque_z: force.z } );
		}
	};

	// Physijs.Mesh.applyCentralForce   MODIFICADO
	Mesh.prototype.applyCentralForce = function ( force ) {
		if ( this.world ) {
			this.world.execute( 'applyCentralForce', { id: this._physijs.id, x: force.x, y: force.y, z: force.z } );
		}
	};

	// Physijs.Mesh.applyForce   MODIFICADO
	Mesh.prototype.applyForce = function ( force, offset ) {
		if ( this.world ) {
	...
	// Physijs.Mesh.getAngularVelocity   MODIFICADO
	Mesh.prototype.getAngularVelocity = function () {
		return this._physijs.angularVelocity;
	};

	// Physijs.Mesh.setAngularVelocity   MODIFICADO
	Mesh.prototype.setAngularVelocity = function ( velocity ) {
		if ( this.world ) {
			this.world.execute( 'setAngularVelocity', { id: this._physijs.id, x: velocity.x, y: velocity.y, z: velocity.z } );
		}
	};

	// Physijs.Mesh.getLinearVelocity   MODIFICADO
	Mesh.prototype.getLinearVelocity = function () {
		return this._physijs.linearVelocity;
	};

	// Physijs.Mesh.setLinearVelocity   MODIFICADO
	Mesh.prototype.setLinearVelocity = function ( velocity ) {
		if ( this.world ) {
			this.world.execute( 'setLinearVelocity', { id: this._physijs.id, x: velocity.x, y: velocity.y, z: velocity.z } );
		}
	};

	// Physijs.Mesh.setAngularFactor   MODIFICADO
	Mesh.prototype.setAngularFactor = function ( factor ) {
		if ( this.world ) {
			this.world.execute( 'setAngularFactor', { id: this._physijs.id, x: factor.x, y: factor.y, z: factor.z } );
		}
	};

	// Physijs.Mesh.setLinearFactor   MODIFICADO
	Mesh.prototype.setLinearFactor = function ( factor ) {
		if ( this.world ) {
			this.world.execute( 'setLinearFactor', { id: this._physijs.id, x: factor.x, y: factor.y, z: factor.z } );
		}
	};

	// Physijs.Mesh.setDamping   MODIFICADO
	Mesh.prototype.setDamping = function ( linear, angular ) {
		if ( this.world ) {
			this.world.execute( 'setDamping', { id: this._physijs.id, linear: linear, angular: angular } );
		}
	};

	// Physijs.Mesh.setCcdMotionThreshold   MODIFICADO
	Mesh.prototype.setCcdMotionThreshold = function ( threshold ) {
		if ( this.world ) {
			this.world.execute( 'setCcdMotionThreshold', { id: this._physijs.id, threshold: threshold } );
		}
	};

	// Physijs.Mesh.setCcdSweptSphereRadius   MODIFICADO
	Mesh.prototype.setCcdSweptSphereRadius = function ( radius ) {
		if ( this.world ) {
	...
	// Physijs.HeightfieldMesh   MODIFICADO
...
// 	Physijs.SphereMesh.prototype.constructor = Physijs.SphereMesh;
	...
	// Physijs.CylinderMesh   MODIFICADO
...
// 	Physijs.CylinderMesh.prototype.constructor = Physijs.CylinderMesh;
	...
	// Physijs.CapsuleMesh    MODIFICADO
...
// 	Physijs.ConeMesh.prototype.constructor = Physijs.ConeMesh;
	...
	// Physijs.ConcaveMesh   MODIFICADO
...
export { scripts, createMaterial, 
  PointConstraint, HingeConstraint, SliderConstraint, ConeTwistConstraint, DOFConstraint, 
  Scene, Mesh, PlaneMesh, HeightfieldMesh, BoxMesh, SphereMesh, CylinderMesh, CapsuleMesh, ConeMesh, ConcaveMesh, ConvexMesh, 
